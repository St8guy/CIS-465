...
#read in image
im = Image.open('Project-Data/image1.jpg')
im_g = Image.open('Project-Data/image1.jpg')
im_g.show()
...
#now we repeat the process for APT
im = im.quantize(threshold)
im_g = im
im_g.show()
iterator = 0
#Apply a mean smoothing filter to the image
for w in range(width):
    for h in range(height):
        ...
        for kernelW in range(3):
            for kernelH in range(3):
                try:
                    average[iterator] = im_g.getpixel((w-1+kernelW, h-1+kernelH))
                except IndexError:
                    average[iterator] = 0
                iterator = iterator + 1
        iterator = 0
        avg = statistics.mean(average)
        im_g.putpixel((w, h), (int(avg)))
iterator = 0
#make a histogram from the image post-smoothing
for w in range(width):
    for h in range(height):
        avg = im_g.getpixel((w, h))
        hist[iterator] = avg
        values[int(avg)] = values[int(avg)] + 1
        iterator = iterator + 1
im_g.show()
